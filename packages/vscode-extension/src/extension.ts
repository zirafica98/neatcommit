...
async function triggerBranchAnalysis(
  apiUrl: string,
  token: string,
  owner: string,
  repo: string,
...
async function pollReviewUntilComplete(
  apiUrl: string,
  token: string,
  reviewId: string,
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const analyzeBranch = async () => {
    ...
    if (!t) {
      await vscode.window.showWarningMessage(
        'NeatCommit: Set neatcommit.token in settings to run analysis.',
      );
      return;
    }
    const folder = vscode.workspace.workspaceFolders?.[0];
    ...
    try {
      await vscode.window.withProgress(
        ...
        async (progress) => {
          ...
          applyDiagnostics(diagnosticCollection, folder, issues);
          await vscode.window.showInformationMessage(
            `NeatCommit: Found ${issues.length} issue(s) on branch "${branch}".`,
          );
        },
      );
    } catch (e) {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('neatcommit.analyzeBranch', analyzeBranch),
  );
